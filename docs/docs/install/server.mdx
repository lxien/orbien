...
docker run -d \
  ...
  -e TZ=Asia/Shanghai \
  xiaoniucode/etps:0.10.0
```

:::tip 提示
  -p 9050-9099:9050-9099：是TCP隧道可以使用的公网端口，实际根据自己需求调整。
:::
### 1.2、自定义部署
...
services:
  etps-mysql:
    ...
    healthcheck:
      ...
      start_period: 60s
  etps:
    image: xiaoniucode/etps:0.10.0
    container_name: etps
    restart: unless-stopped
    ports:
      - "8080:8080" # HTTP 隧道代理服务
      - "8020:8020" # 管理面板
      - "9527:9527" # 控制隧道
      - "9050-9099:9050-9099" # TCP 隧道代理服务
    environment:
      JAVA_OPTS: "-Xms256m -Xmx256m -XX:MaxDirectMemorySize=512m -XX:+UseG1GC"
      MYSQL_PASSWORD: etps.123456
      MYSQL_HOST: etps-mysql
      TZ: Asia/Shanghai
    volumes:
